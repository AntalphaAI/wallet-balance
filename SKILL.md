...
- **MCP aggregation**: The gateway calls `https://mcp-skills.ai.antalpha.com/mcp` by default and invokes **`multi-source-token-list`** (**EVM addresses only**). The host must reach that environment; adjust with `MCP_SKILLS_URL` / `ENABLE_MCP` (see `README.md` and `.env.example`).
- **Fallback**: If MCP times out, is unreachable, or returns an error, and `ENABLE_FALLBACK_PROVIDER` is true (default), EVM queries use a **public** subset (e.g. ETH/BSC native and some USDT; see `server.js`). **BTC** always uses public sources (e.g. Blockstream) and never MCP.
- If `data_source` is `public_only` or `public_fallback`, you may briefly say results are from a **public-query scope** (in the user’s language); for `mcp_aggregate`, emphasizing the source is optional.
